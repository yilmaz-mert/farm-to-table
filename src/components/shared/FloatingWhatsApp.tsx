...
export function FloatingWhatsApp() {
  ...
  function handleClick() {
    ...
    const url = `https://wa.me/${FARMER_PHONE}?text=${encodeURIComponent(text)}`
    ...
    >
      ...
      <AnimatePresence>
        {hasItems && (
          <motion.span
            key="badge"
            className="absolute -right-1 -top-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-cta px-1 font-mono text-[10px] font-bold text-inverted"
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            exit={{ scale: 0 }}
